...
class Classifier_Trainer():
    ...
    def get_pseudo_labels(self, unlabeled_ds, threshold=0.65, num_workers=0,batch_size=4):
        ...
        if len(aug_images) > 0:
            ...
            # aug_images, aug_labels = aug_images,aug_labels.cpu()
            # print(aug_images.shape,aug_labels)
    ...
    def train(
        ...
        total_steps = 0
        res_dir = logger.getdir_artifact() if logger is not None else './results'
        for epoch in range(num_epochs):
            ...
            if ds_balanced:
                if unlabeled_ds is None:
                    ...
                    pseudo_set = self.get_pseudo_labels(unlabeled_ds, threshold,batch_size=batch_size)
                ...
                img_dl = DataLoader(train_ds, batch_size=batch_size, sampler=sampler, num_workers=num_workers,
                                    pin_memory=True)
            else:
                if unlabeled_ds is None:
                    train_ds = img_ds
            ...
            for i, (images, labels) in enumerate(img_dl, start=1):
                ...
                if (total_steps + 1) % cpt_frequency == 0:
                    print('saved model')
                    self.save(os.path.join(res_dir, 'model.pth'))
                print(
                    f"epoch:{epoch} batch:{i}  loss: {loss.item():f} train_acc: {train_acc:f} valid_acc: {valid_acc:f}")

                if logger is not None:
                    logger.log_metric('loss', loss.item())
                    logger.log_metric('train_acc', train_acc.item())
                    if total_steps % valid_frequency == 0 and valid_ds is not None:
                        logger.log_metric('valid_acc', valid_acc.item())

                total_steps += 1

    def test(self,img_ds,unlabeled_ds):
        optimizer = torch.optim.Adam(self.model.parameters(), 3e-4)
        for epoch in range(10):
            pseudo_set = self.get_pseudo_labels(unlabeled_ds, 0.01,batch_size=16)
            # train_ds = ConcatDataset([img_ds, pseudo_set]) if pseudo_set is not None else img_ds
            for image,label in pseudo_set:print(image.shape,label)
            for image,label in img_ds:print(image.shape,label)
            train_ds = (img_ds + pseudo_set) if pseudo_set is not None else img_ds
            img_dl = DataLoader(train_ds, batch_size=16, shuffle=True, num_workers=0,pin_memory=False)
            # ----------------------------开始训练--------------------------- #
            for i, (images, labels) in enumerate(img_dl, start=1):
                self.model.train()
                x, y = images.to(self.device), labels.to(self.device)
                ...
                # optimizer.step()
                # print(
                #     f"epoch:{epoch} batch:{i}  loss: {loss.item():f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ds_balanced', action='store_true', default=False)
    ...
    if not args.use_unlabeled: unlabeled_set = None
    ...
    if args.logger:
        ...
        logger = Mlflow_Logger(experiment_name='food_11')
        logger.log_params(vars(args))
    else:
        logger = None

    classifier_trianer = Classifier_Trainer(model, device=args.device)
    classifier_trianer.test(train_set,unlabeled_set)
