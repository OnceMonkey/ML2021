...
class Classifier_Trainer():
    ...
    def get_pseudo_labels(self, unlabeled_ds, threshold=0.65, num_workers=0,batch_size=4):
        ...
        if len(aug_images) > 0:
            aug_images, aug_labels = torch.stack(aug_images), aug_labels
            semi_ds = Custom_Dataset(aug_images, aug_labels)
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            if ds_balanced:
                ...
                img_dl = DataLoader(train_ds, batch_size=batch_size, sampler=sampler, num_workers=num_workers,pin_memory=True)
            else:
                if unlabeled_ds is None:
                    train_ds = img_ds
                else:
...
if __name__ == '__main__':
    ...
    # classifier_trianer.load('./results/model.pth')
